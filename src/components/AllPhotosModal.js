    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 overflow-auto">
      <div className="bg-white rounded-lg p-4 max-w-4xl w-full mx-4 shadow-xl border border-red-600">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold text-red-700">All Photos</h2>
          <button onClick={onClose}>
            <XCircleIcon className="w-8 h-8 text-red-500 hover:text-red-700 transition-colors" />
          </button>
        </div>
            ...
            {images.map((img) => (
              <div key={img.id} className="relative border rounded overflow-hidden shadow-lg">
                <div className="relative h-40 w-full">
                  <Image src={img.image_url || '/default-image.jpg'} alt="User Image" fill className="object-cover rounded" />
                </div>
                <div className="p-2">
                  <p className="text-sm text-gray-600">Added: {new Date(img.created_at).toLocaleDateString()}</p>
